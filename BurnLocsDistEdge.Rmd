...
points(x = summer_2022_burned_locs_burned_and_unburned$easting, 
       y = summer_2022_burned_locs_burned_and_unburned$northing, 
       col = custom_palette_points[summer_2022_burned_locs_burned_and_unburned$LCOV], 
       pch = 20, 
       cex = 0.3)
#This looks good

#A lot of unnecessary columns. Let's refine and make it a tibble since its 2023.
summer_2022_burned_locs_clean <- summer_2022_burned_locs %>% 
  filter(LCOV != "AG") %>% 
  as_tibble() %>%
  dplyr::select(title, date_creat, date, northing, easting, band_numb, trap_locat, LCOV) %>%
...
summer_2022_burned_locs_burned_and_unburned$DSF = as.numeric(summer_2022_burned_locs_burned_and_unburned$date - burn_date_2022)

summer_2022_burned_locs_burned_and_unburned %>%
  arrange(DSF) %>%
  view()

#Fitting glm
prob_burned_area <- glmer(burned ~ DSF + (1|band_numb),
                 data = summer_2022_burned_locs_burned_and_unburned,
                 na.action = "na.pass",
                 family = "binomial")
