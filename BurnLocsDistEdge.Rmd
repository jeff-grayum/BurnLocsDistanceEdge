...
Lets run a mix-ed model and see if probability of locating a bobwhite in a burned area depends on DSF
```{r}
summer_2022_burned_locs_burned_and_unburned <- read_xlsx("/Volumes/Samsung_T5/BOBWHITE_DATA/Clean/Sum2022BurnedDist.xlsx") %>%
  clean_names() %>%
  filter(status != "D") %>%
  mutate(date = as.Date(date)) %>%
